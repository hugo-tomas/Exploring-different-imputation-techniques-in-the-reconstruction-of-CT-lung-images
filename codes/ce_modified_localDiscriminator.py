...
class _netlocalD(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.linear.in_features != output.size(1):
            self.linear = nn.Linear(in_features=output.size(1), out_features=1).to(device)
...
class context_encoder():
  ...
  def train(self, fold, dataloader,opt):
    ...
    pbar_epochs.set_description("TRAINED EPOCHS | CONTEXT ENCODER [MODIFIED - LOCAL DISCRIMINATOR]")
    ...
    for epoch in range(resume_epoch,opt.epochs+1):
      for i, data in enumerate(dataloader, 0):
        ...
        output = netD(processor.crop(fake,crop_masks))
        ...
        netD.zero_grad()
        output_real = netD(processor.crop(input_real, crop_masks))
        D_x = output_real.data.mean()

        # Train D with fake image
        output_fake = netD(processor.crop(fake, crop_masks).detach())
